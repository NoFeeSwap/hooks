...
def mintIncentiveSequence(nofeeswap, incentive, token0, token1, tagShares, poolId, qMin, qMax, shares, hookData, deadline):
    sharesSlot = 1
    ...
    sequence[8] = encode_packed(
      ['uint8', 'uint256', 'address', 'uint8', 'uint8'],
      [TRANSFER_TRANSIENT_BALANCE, tagShares, incentive.address, sharesSlot, sharesSuccessSlot]
    )
    
    return encode_packed(['uint32'] + ['bytes'] * len(sequence), [deadline] + sequence)

def burnIncentiveSequence(token0, token1, payer, operator, tagShares, poolId, qMin, qMax, shares, hookData, deadline):
    sharesSlot = 1
    ...
    sequence[7] = encode_packed(
      ['uint8', 'uint256', 'address', 'uint8', 'uint8'],
      [TRANSFER_TRANSIENT_BALANCE_FROM_PAYER, tagShares, operator.address, sharesSlot, sharesSuccessSlot]
    )
